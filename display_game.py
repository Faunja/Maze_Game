...
def draw_maze(Maze, offset, color):
	...
	for row in range(len(Maze.maze)):
		yPosition = row * displayboxSize - offset[1] + Display.ScreenOffset[1]
		if -displayboxSize > yPosition or Display.DisplayHeight < yPosition:
			continue
		for column in range(len(Maze.maze)):
			xPosition = column * displayboxSize - offset[0] + Display.ScreenOffset[0]
...
def display_memoryGrid():
		...
		for y in range(-1, 2):
			...
			for box in Character.storedPositions[yPosition][xPosition]:
				...
				if xOffset < -displayboxSize or xOffset > Display.DisplayWidth + displayboxSize:
					continue
				yOffset = box[1] * displayboxSize - (displaycameraPosition[1] - (yPosition - Character.startgridPosition[1]) * Grid.mazeSize * displayboxSize) + Display.ScreenOffset[1]
	...
	for currentPosition in Character.currentPositions:
		...
		xOffset = currentPosition[1][0] * displayboxSize - (displaycameraPosition[0] - (currentPosition[0][0] - Character.startgridPosition[0]) * Grid.mazeSize * displayboxSize) + Display.ScreenOffset[0]
		if xOffset < -displayboxSize or xOffset > Display.DisplayWidth + displayboxSize:
			continue
		yOffset = currentPosition[1][1] * displayboxSize - (displaycameraPosition[1] - (currentPosition[0][1] - Character.startgridPosition[1]) * Grid.mazeSize * displayboxSize) + Display.ScreenOffset[1]
		if yOffset < -displayboxSize or yOffset > Display.DisplayHeight + displayboxSize:
			continue
